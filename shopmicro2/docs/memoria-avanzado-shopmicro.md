...
### 9.1. Fase 1 - Ejercicio 1 - Escalado dinámico por servicio

Estado: `Ejecutado`

Objetivo operativo:

- escalar `shopmicro_product-service` de 2 a 4 réplicas;
- escalar `shopmicro_order-service` de 2 a 3 réplicas;
- documentar distribución de tareas en los nodos del clúster Swarm.

Acciones preparadas en el repositorio:

- actualización de la guía [swarm-commands.md](/C:/Users/G513/OneDrive%20-%20Sa%20Palomera/Documentos/Codex%20Projects/Kubernetes%20Avanzado/shopmicro/scripts/swarm-commands.md);
- creación del documento operativo [swarm-fase1-ejercicio1.md](/C:/Users/G513/OneDrive%20-%20Sa%20Palomera/Documentos/Codex%20Projects/Kubernetes%20Avanzado/shopmicro/docs/swarm-fase1-ejercicio1.md).

Resultado registrado:

- `shopmicro_product-service` escalado correctamente a `4/4`;
- `shopmicro_order-service` escalado correctamente a `3/3`;
- distribución efectiva entre `swarm-worker-1`, `swarm-manager` y `swarm-worker-2`;
- evidencia recibida de `docker service ps` y `docker stack services`.

Lectura técnica:

- el clúster ha respondido correctamente al escalado por servicio sin necesidad de redeploy completo;
- el comportamiento observado valida el primer requisito de la Fase 1 avanzada.

Incidencia abierta detectada durante la validación:

- `shopmicro_message-queue` aparece en estado `0/1`.

Decisión operativa:

- el ejercicio 1 se considera superado;
- antes de profundizar en ejercicios posteriores de Swarm conviene revisar la salud de `message-queue`, porque afectará a pruebas funcionales y a validaciones más exigentes.
